...
class SourceFile {
    ...
    constructor(language, name, source) {
        ...
        if(tokenizer.getErrors().length === 0) {
            ...
            if(parser.getErrors().length === 0)
                this._instructions = this._ast.serialize();
            // Otherwise, grab the errors from the parser.
            else
                this._parsingErrors = parser.getErrors();
    ...
    getText(startIndex, stopIndex) {
        ...
        if(startIndex < 0 || startIndex >= this._source.length || stopIndex < 0 || stopIndex >= this._source.length) {
            ...
            return this._source.substring(startIndex, stopIndex + 1);
...
class Tokenizer {
    ...
    setCurrentIndex(index) { this._currentIndex = index; }

    getCurrentIndex() { return this._currentIndex; }

    getSourceFile() { return this._file; }
     ...
     */
    addToken(token) { this._tokens.push(token); }
    ...
    peek(numberAhead) {
        ...
        if(peekIndex >= this._source.length)
            return null;
        else
            return this._source[peekIndex];
    ...
    next() {
        ...
        if(ch === null)
            throw new ParsingError(this.getLastToken(), this._eofError);
        ...
        while(this.getCurrentIndex() < this.getSource().length) {
            ...
            try {
                ...
                if (previousIndex === this.getCurrentIndex())
                    throw "Defect in tokenizer; index didn't advance past character " + this.getCurrentIndex() + " '" + this.peek() + "'";
                else
...
class Tokens {
    ...
    setCurrentIndex(index) { this._index = index; }
    ...
    hasNext() { return this._index < this._tokens.length; }
    ...
    consume(type, text) {
        ...
        if(next === null) {
            if(text)
                throw new ParsingError(this.peek(), "Expected " + type + " '" + text + "'");
            else
                throw new ParsingError(this.peek(), "Expected " + type);
        }
...
class ErrorAST {
    ...
    getProblematicToken() { return this._problematicToken; }
...
class Parser {
    ...
    getErrors() { return this._parsingErrors; }
...
class Instruction {
    ...
    getFirstToken() {
        ...
        var sortedTokens = _.sortBy(
            _.filter(
                Object.getOwnPropertyNames(this),
                (name) => { return this[name] instanceof Token; }
            ),
            (name) => { return this[name].getStartIndex(); }
        );
     ...
     */
    getValueDependencies(runtime) { return []; }
...
class Runtime {
     ...
     */
    getBreakpointSteps() { return this._breakpointSteps; }

    /**
     * @returns {number} Number of the current step.
     */
    getCurrentStepCount() { return this._stepCount; }
    ...
    incrementStepCount() {
        ...
        this._stepCount++;
        if(this._stepCount > this._maxStepCount)
            this._maxStepCount = this._stepCount;
    ...
    onBreakpoint() {
        ...
        if(onBreakpoint)
            this._breakpointSteps[this.getCurrentStepCount()] = true;
     ...
     */
    getMaxStepCount() { return this._maxStepCount; }

    /**
     * @returns {{}} The steps on which output occurred.
     */
    getOutputSteps() { return this._outputSteps; }
    ...
    notifyListeners() {
        ...
        _.each(this._listeners, (listener) => { listener.changed(); });
...
class ParsingError {
    ...
    getMessageAndContext() {
        ...
        var context =
            this._token == null ?
                "Beginning of file" :
                this._token.getFile().getText(Math.max(0, this._token.getStopIndex() - 100), this._token.getStopIndex());

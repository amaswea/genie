...
class JavaScriptFile extends SourceFile {
    ...
    _createParser(tokens) {
        ...
        for(let i = 0; i < tokens.length; i++) {
            ...
            let token = tokens[i];
            let nextParsableTokens = _.filter(tokens.slice(i), (token) => { return !token.isComment() && !token.isWhitespace(); });
            ...
            if(!token.isWhitespace() &&
                (
                    ...
                    nextParsableToken.getText() === "function"
                )
            )
                parsableTokens.push(token);
